...
<div class="calendar_wrap5">
	<div class="year_wrap">
		<p class="cal_pre"><img src="/image/common/cal_arrow_left.gif" alt="이전달" onclick="<?if($prev_view){?>axCalList3('/js/Calendar/ResCal_admin.php?currMonth=<?=$prevMonth?>&currYear=<?=$prevYear?>&InDate=<?=$InDate?>&b_uid=<?=$b_uid?>&m_uid=<?=$m_uid?>&s_uid=<?=$s_uid?>&Module=<?=$Module?>&g_uid=<?=$g_uid?>&obj=<?=$obj?>','<?=$obj?>','<?=$Module?>');<?}?>return false;" style="cursor:pointer;" /></p>
		<p class="cal_year"><?=$currYear?>년&nbsp;<?=$currMonth?>월</p>
		<p class="cal_next"><img src="/image/common/cal_arrow_right.gif" alt="다음달" onclick="<?if($next_view){?>axCalList3('/js/Calendar/ResCal_admin.php?currMonth=<?=$nextMonth?>&currYear=<?=$nextYear?>&InDate=<?=$InDate?>&b_uid=<?=$b_uid?>&m_uid=<?=$m_uid?>&s_uid=<?=$s_uid?>&Module=<?=$Module?>&g_uid=<?=$g_uid?>&obj=<?=$obj?>','<?=$obj?>','<?=$Module?>');<?}?>return false;" style="cursor:pointer;" /></p>
	</div>
